...
reference evidence in [VERIFICATION.md](VERIFICATION.md).
...
assign a qualified specialist and explicit write scope; the existing role
...
[coverage-report](VERIFICATION.md) is diagnostic and does not enforce it.
...
## Reusable check register

These stable IDs help instantiate the stages above; they are **manual review
obligations**, not new implemented gates. For each applicable ID create or link
one scoped record with owner, independent reviewer, authorized approver,
candidate/configuration, requirements, measurable acceptance, method/tool and
raw evidence. Split a row by instance/profile when its assumptions differ.
Freeze thresholds, seeds, corner/mode matrices and workload selection before
execution. An undefined target or missing qualified tool remains OPEN/BLOCKED.
A manual review does not replace a required machine analysis merely because
its tool is unavailable. Apply the independently authorized N/A or waiver
process above; do not silently remove an ID or weaken an existing requirement.

The criteria are architecture-dependent: CPU ISA, external memory, ROM,
provisioning, DFT and board obligations are instantiated where required by the
approved product contract. This register prescribes neither a particular NPU,
bus, memory size, numerical tolerance, shuttle nor tool vendor. Tools used for
imported IP must match its approved native flow. Stage 7 release reconciles all
applicable **pre-tapeout** obligations; stage 8 measurements remain planned until
real silicon exists. Do not demand post-silicon PASS before tapeout or infer it
from simulation.

### Feasibility and architecture — stages 0–1

| ID | Evidence and decision required |
|---|---|
| FEAS-01 | Target process/package delivery rules, usable PDK/IP/tool licenses and qualified signoff route identified |
| FEAS-02 | Pin/IO/electrical budget fits package, board, external devices, test and debug requirements |
| FEAS-03 | Required memory capacity/ports/timing mapped to feasible macros or reviewed implementations |
| FEAS-04 | Nonvolatile/ROM image delivery, update and initialization strategy feasible for the chosen process |
| FEAS-05 | Required external-memory controller/PHY/device interfaces available with defined bandwidth and timing assumptions |
| FEAS-06 | Clock, reset, power and startup sources have realizable implementation and operating limits |
| FEAS-07 | Calibrated area/power/performance estimates meet frozen limits and justified margins |
| FEAS-08 | Required recovery, fallback and observability paths remain usable after plausible failures |
| FEAS-09 | Package/board/off-chip devices, test resources, cost and schedule fit the agreed system budget |
| ARCH-01 | Behavior and acceptance contract frozen with source/version hashes and approval |
| ARCH-02 | Requirements trace to implementation owners, independent tests/properties and coverage obligations |
| ARCH-03 | Numerical formats, rounding, overflow, accuracy metrics and tolerances are unambiguous |
| ARCH-04 | Interfaces define ordering, latency/progress, backpressure, errors, reset and concurrency semantics |
| ARCH-05 | Imported IP revision, license, dependencies, supported configuration and integration assumptions reviewed |
| ARCH-06 | Resource and traffic budgets calibrated at stated model fidelity, with unproven assumptions explicit |
| ARCH-07 | Security, provisioning, privilege, debug/test and recovery scope defined with negative cases |
| ARCH-08 | Behavior changes use reviewed change orders and downstream invalidation/rerun decisions |
| OBS-01 | Required signatures, counters, error visibility and debug access make acceptance and failure triage observable without bypassing production behavior |

### Block readiness and verification — stages 2–3

| ID | Evidence and decision required |
|---|---|
| RTL-01 | Applicable native lint and synthesis-language checks pass with independently reviewed dispositions |
| RTL-02 | Elaboration and storage/net structure match supported configurations; no unintended latches or infeasible inferred memories |
| RTL-03 | Clock/reset/power domain inventory and reset/startup behavior checked with suitable analysis |
| RTL-04 | Exact instance parameters, generated sources, defines and IP configurations match the accepted candidate |
| RTL-05 | Provisional behavior, specification gaps and serious implementation bugs resolved before the claimed readiness level |
| RTL-06 | Port/decode/connectivity checks establish required paths and no unintended drivers or unconnected obligations |
| RTL-07 | Imported sources remain pinned; adaptation diff and license/source notices retained |
| RTL-08 | Memory/macro mapping and early synthesis/timing/area feasibility meet the assigned block budget |
| DV-01 | Independently authored spec-derived plan, oracle and checkers have traceable expected behavior |
| DV-02 | Every applicable requirement has executed checking plus its planned activation/coverage evidence; a mapped row or hit bin alone is insufficient |
| DV-03 | Functional and code coverage close against approved denominators, profile membership, exclusions and thresholds |
| DV-04 | Required negative, boundary, backpressure, concurrent and stress cases check outcomes and progress |
| DV-05 | Approved profiles and random seed set/count actually execute with reproducible raw receipts; skipped and unchecked runs remain explicit |
| DV-06 | Checkers demonstrate required detectability through reviewed validation/negative controls; mutation only where justified by the verification plan, never mandatory defect injection to rank agents |
| DV-07 | Must-fix bugs closed with independent regression evidence; permitted residual risks have named disposition and approval |
| FV-01 | Applicable protocol/safety/progress properties checked under justified environment assumptions |
| FV-02 | Applicable arithmetic/state invariants checked at the claimed parameter/configuration scope |
| FV-03 | Activation, reachability, vacuity, abstraction, assumptions, bounds/proofs and unknown/timeouts reviewed; parent environments discharge reusable assumptions |
| ISA-01 | Implemented ISA/extensions/configuration checked against the approved independent architectural reference and compliance suite |
| ISA-02 | Custom instructions, traps, privilege and deviations have explicit contracts and independent tests |

### Subsystem, SoC and software — stages 4–5

| ID | Evidence and decision required |
|---|---|
| SYS-01 | Production-intent reset and boot path reaches checked software completion without undisclosed preload/stub bypasses |
| SYS-02 | Required end-to-end system paths and contention check data integrity, ordering and bounded progress |
| SYS-03 | Frozen workload passes its accuracy/performance/resource criteria at the approved execution level and scope; proxies never count as the real workload |
| SYS-04 | Required fault, reset, corrupted-input, security and recovery scenarios reach checked outcomes |
| SYS-05 | Contract-required fallback, update and degraded operation work at their real system boundaries |
| SYS-06 | Top-level requirement/coverage closure discharges conditional child approvals for exact instances, modes and dependencies |
| SW-01 | Toolchain, sources, dependencies, build flags, binaries and loading paths have reproducible identities |
| SW-02 | Independent expected outputs and checker semantics validate real software execution, not just completion |
| SW-03 | Required ROM/constants match source and delivered bytes through synthesis/layout and the actual read path |
| SW-04 | Alternate loading/recovery paths meet the contract where required; absence is not silently waived |

### Implementation, physical checks and release — stages 6–7

| ID | Evidence and decision required |
|---|---|
| IMPL-01 | Final memories/IP/macros, power intent and libraries match the qualified implementation inventory |
| IMPL-02 | Required RTL/netlist/post-DFT/post-route/ECO equivalence comparisons close with assumptions and unmatched partitions reviewed |
| IMPL-03 | Setup/hold, clocks, IO and electrical timing checks pass the complete approved corner/mode/extraction matrix |
| IMPL-04 | Timing exceptions and disabled/unconstrained paths are justified, bounded and independently approved |
| IMPL-05 | CDC/RDC, four-state X/startup and low-power analysis close with capable tools and discharged assumptions; two-state simulation is insufficient for X closure |
| IMPL-06 | Required activity-bound power, IR and electromigration analyses meet qualified limits; static IR alone proves neither dynamic IR nor full EM closure |
| IMPL-07 | DFT insertion, ATPG/scan and memory-test evidence meet approved manufacturing fault models/coverage and test-access constraints; functional coverage is separate |
| PHY-01 | Final layout DRC passes the target's approved decks with authorized dispositions |
| PHY-02 | Final extracted connectivity/LVS matches the accepted netlist and macro interfaces |
| PHY-03 | Required antenna, density, ERC and other process/electrical checks pass |
| PHY-04 | Foundry/package submission prechecks and qualification obligations are discharged using the accepted flow |
| PHY-05 | Netlist/layout/extraction/constraints/PDK/deck/library/tool identities and checksums bind every physical result to the delivered candidate |
| PHY-06 | Preapproved decision-bearing same-binary post-layout suite passes with required SDF annotation and checked results; omitted software obligations remain open unless separately authorized |
| PHY-07 | Required programmed/constant data survives delivery transformations and readback checks |
| PHY-08 | IP, process, software and deliverable licenses/notices permit the intended publication, manufacture and distribution |
| REL-01 | Candidate ledger reconciles all applicable pre-tapeout criteria, dependencies, reruns and exceptions; pending is not PASS |
| REL-02 | Independent release and qualified discipline reviewers approve the exact candidate and evidence |
| REL-03 | No must-fix bug remains; residual risks and authorized exceptions name impact, owner, mitigation and approval |
| REL-04 | Maintainer's explicit final authorization identifies artifact hashes, target recipient and submission scope |
| BRD-01 | Pinout, electrical limits, power/reset/clock requirements and package/board interfaces agree |
| BRD-02 | Board/fixture/BOM and manufacturing-test resources support the required bring-up and production checks |
| BRD-03 | Silicon validation procedures, acceptance limits and triage ownership exist before hardware arrives |

### Silicon and cross-cutting evidence — stage 8 and every stage

| ID | Evidence and decision required |
|---|---|
| SIL-01 | Actual devices meet power, clock, reset and basic electrical startup criteria |
| SIL-02 | Device/lot/revision identity and required self-tests are recorded and checked |
| SIL-03 | Real boot, provisioning and firmware paths work on identified boards/devices |
| SIL-04 | Required system/IO/memory scenarios pass on hardware with checked outcomes |
| SIL-05 | Actual workload meets frozen correctness and performance criteria at measured conditions |
| SIL-06 | Required negative/fault/recovery cases pass without hiding affected units or failures |
| SIL-07 | Voltage, temperature, frequency and other operating margins satisfy the qualification plan |
| SIL-08 | Required soak, reliability and production sampling complete with failures and denominators reported |
| SIL-09 | Measured power/performance and physical area/package evidence reconcile with budgets and model limits |
| SIL-10 | Manufacturing-test correlation, yield, errata and failure triage support the claimed production readiness |
| EVID-01 | Raw receipts bind inputs, environment, commands, exit status and semantic verdict; planned/selected/executed/skipped/checked scope is explicit |
| EVID-02 | Author, independent reviewer and approver have distinct authorized roles and identified evidence contributions |
| EVID-03 | Add/modify/delete impact traverses specification, model, DV, software and physical consumers; affected candidates reopen, historical records remain, unaffected evidence carries forward only by reviewed justification |
| EVID-04 | Requested model/effort and independently observed identity remain separate; unavailable attestation is UNKNOWN, not inferred from vendor or agreement |
| EVID-05 | N/A and waivers use their distinct independent authorization and rationale; serious bugs, undefined acceptance and missing checks cannot disappear into a summary count |
