...
Version 1.0-rc4 (change order rc4, proposal pending rc3-reviewer confirmation) · 2026-09-05 intake · reviewed external-IP metadata from [intake matrix](intake/ip-ip_matrix.json) and [recommendation](intake/ip-RECOMMENDATION.md). Architect did not read implementation RTL or modify it. The evidence below is the intake owner's reported direct probe result, not a new architect-run test.
...
Imported CPU register-file reset and Verilog constructs may conflict with project-wide every-flop-reset/style rules. This is a surfaced adoption issue, not a blanket waiver: RTL/integrator must resolve scoped imported-IP conventions with explicit evidence before gate acceptance. Firmware initializes registers/state it observes; q2/q3 are scratch and not assumed zero. rc4 (SYS-13, ISSUE-cpu_bridge-01): the vendored `hw/ip/picorv32/README.md` (sha256 7f3259f25867968126e5cb560c690f97058d9be2fa1d368b69bab26861e903c5, upstream commit a473fc8fca393771d83b0ffcf0b14db3393339d8) sections "PicoRV32 Native Memory Interface" and "Custom Instructions for IRQ Handling" are normative references for the native port and IRQ ABI that DV may read (where they and SYS-13 differ, SYS-13 governs); `picorv32.v` stays implementation-only and is not a DV input. FENCE is decoded, FENCE.I is unsupported; boot copies executable SRAM before initial jump and forbids self-modifying code. PicoRV32 instruction prefetch exists even without caches, so dynamic instruction rewrites are not safe by assumption.
