...
Version 1.0-rc3. Requirement IDs are immutable traceability keys. A shall applies to SIM-L1 unless marked S1 or P1. Addresses are byte addresses, ranges half-open, sizes binary. All control values and tensor bytes are little-endian.
...
| 0x08 | BOOT_STAGE | RW / 0 | 0 reset, 1 loader, 2 firmware, 3 B1, 4 L1; 0..4 only |
...
| 0x18 | RESULT_COMMIT | WO / 0 | write1 latches RESULT_CODE to output and sets o_result_valid until reset; additional commits rejected |
...
UART 0x40003000: offset0 TX_DATA WO bits7:0; offset4 STATUS RO bit0 READY, bit1 BUSY; offset8 DIVISOR RW reset16, legal 2..65535. READY=1 when one-byte holding slot empty; TX_DATA when not ready returns SLVERR (not silently dropped). Divisor writes while BUSY reject. Idle TX=1. **SYS-11:** UART shall serialize one start-zero, 8 data LSB-first and one stop-one, each DIVISOR clocks; a holding byte transfers to the shifter whenever idle, freeing the holding slot. IRQ TX_EMPTY is READY. UART is diagnostic output; no RX, receive IRQ, baud tolerance or FIFO deeper than one is promised.
