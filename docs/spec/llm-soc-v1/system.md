...
**SYS-08:** All CSR bus transactions shall be aligned 32-bit single transfers with WSTRB=0xf on writes. Software must issue aligned word CSR loads/stores; the native CPU port does not expose original read size/byte offset, so hardware cannot distinguish a CPU LB/LH from LW of the same word. Such subword CSR reads are a software ABI violation, not a promised hardware rejection. Other sizes, strobes, offsets or writes to RO registers return SLVERR, reads return zero on error and no state changes. Undefined bits read zero; reserved write bits must be zero or the write is rejected atomically. RO means no writes, RW normal replacement, W1C clears written-one bits, WO reads return zero without side effects. Unless explicitly listed, reset is zero. If a hardware event and W1C coincide, event wins. CSR transactions never depend on a later software request to complete. WO command registers (RESULT_COMMIT, NPU SUBMIT, NPU CLEAR) accept exactly the written word value 1: any other written word, including 0, returns SLVERR and has no effect; TX_DATA accepts any word whose bits 31:8 are zero (this restates the reserved-write-bits rule above for TX_DATA; it adds no uart obligation) (rc4). Value ranges stated in the sys and UART register rows are hardware-enforced: a BOOT_STAGE write with value above 4 and a DIVISOR write with field value below 2 return SLVERR and change nothing; NPU descriptor shadow registers accept any word value while writable (NPU-04) and are validated only at SUBMIT (NPU-05) (rc4). Every SLVERR or DECERR returned to a CPU CSR access, including each write rejection this paragraph enforces (WO write not equal to 1, BOOT_STAGE above 4, DIVISOR below 2, RESULT_COMMIT repeat, reserved bits, RO writes), is fatal by SYS-07: the bridge returns no mem_ready, the offending load/store never retires and SYS-12 places the core in local reset. This is intended: a rejected CSR access is a firmware defect that halts the run at the offending instruction; it is never a software-observable or retryable error, and firmware shall not code a check-response-and-continue path around any CSR access (rc4, SW-RC4-01).
