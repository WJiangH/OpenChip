...
4. **Reconcile each relevant job.** Prefer scheduler/run IDs with host and
   ...
   An alive or quiet process is not PASS. Missing terminal evidence means
...
Use a compact update: what changed; what evidence supports it; what blocks the
goal; what happens next. Do not report every unchanged poll. Before ending a
